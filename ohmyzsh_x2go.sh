...
echo "duellj"
nano .zshrcexec
omz plugin load history-substring-search keychain magic-enter npx perl pip please poetry penv edis-cli rust safe-paste 
omz plugin load supervisor sublime symfony textmate thor universalarchive yii ansible arcanist autoenv bundler 
omz plugin load command-not-found composer copyfile docker-machine encode64 fancy-ctrl-z firewalld
omz plugin load git it-hubflow 

omz plugin load lassfish golang grails  gulp heroku hitchhiker httpie ipfs kitchen knife_ssh last-working-dir mysql-macports operator-sdk 
omz plugin load pass pipenv postgres powify pylint rake rbenv repo rsync ssh-agent sublime-merge svn-fast-info symfony2 taskwarrior 
omz plugin load terraform thefuck tmux-cssh torrent ubuntu urltools vault virtualenv vscode xcode zbell zsh-interactive-cd genpass gpg-agent 
omz plugin load lpass python shell-proxy systemadmin timer transfer virtualenvwrapper web-search zsh-navigation-tools gitignore 
omz plugin load screen systemd vagrant-prompt
